## dull-ish

#### A low contrast, minimalistic theme for Neovim.

This theme is based on my own preferences. Low contrast and predominantly monochrome, with some subtle / dull colour hints to help navigate the eyes. It is used as the default theme in the [`dynamic-theme.nvim`](https://github.com/alistairjoelquinn/dynamic-theme.nvim) plugin which allows you to edit the theme from within the plugin window.

##### JavaScript / TypeScript:

<img width="737" alt="Screenshot 2025-05-25 at 14 29 23" src="https://github.com/user-attachments/assets/47ea0cd3-422c-48bd-94ad-7aa97121f3f7" />

##### Rust:

<img width="1174" alt="Screenshot 2025-05-25 at 14 32 56" src="https://github.com/user-attachments/assets/4de7f210-2680-424b-a926-a062660f9bde" />
